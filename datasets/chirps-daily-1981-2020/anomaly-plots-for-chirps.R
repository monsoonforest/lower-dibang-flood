...
longtermfilter_daily <- lapply(daily_rain, function(x,y,...){x %>% filter(year>2000) %>% filter(year<2016)})
...
departure_plot <- function(P,TITLE,...) {ggplot(P %>% filter(date_time >= as.Date('2015-08-01') & date_time <= as.Date('2015-09-30'))) +
...
ggsave(filename="anpum-rainfall-august-september-2015.svg", plot=p, device="svg" ,height=10, width=40,dpi=300,units="cm")
